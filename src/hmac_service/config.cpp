...
namespace service::config
{
  ...
  public:
    ...
    std::string GetLogFile() const { return body_.log_file_; }
  ...
  private:
    struct ConfigBody
    {
      ...
      explicit ConfigBody(std::string hmac, std::string listen,
                          std::string log_level, std::string log_file, uint32_t max_msg_size_bytes,
                          std::string secret)
          : hmac_algo_(std::move(hmac)), listen_(std::move(listen)),
            log_level_(std::move(log_level)), log_file_(log_file),
  ...
  std::string Config::GetLogFile() const { return pImpl->GetLogFile(); }
